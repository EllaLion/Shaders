...
{    static int i = 0;
	...
	glm::mat4x4 view = glm::lookAt(lightPos, vec3(0, 0, 0), vec3(0, 1, 0));
	glm::mat4x4 projection = glm::perspective(80.0f, 1.0f, 1.0f, 50.0f);
...
void SceneShadowMap::compileAndLinkShader()
{
	...
	try {
		prog.compileShader("shadowmap.vert");
    	prog.compileShader("shadowmap.frag");
    	prog.link();
    	prog.validate();
    	prog.use();
