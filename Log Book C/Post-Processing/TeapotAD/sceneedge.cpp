...
void SceneEdge::initScene(QuatCamera camera)
{
	...
	Bitmap mbit = Bitmap::bitmapFromFile("TEX.PNG");
	mbit.flipVertically();

	Texture* mTextureTex = new Texture(mbit);

	gl::ActiveTexture(gl::TEXTURE11);
	gl::BindTexture(gl::TEXTURE_2D, mTextureTex->object());
	GLint loc = gl::GetUniformLocation(prog.getHandle(), "Texture");
	gl::Uniform1i(loc, 11);
